...
python scripts/lark_cli_json.py \
  --json-arg params='{"token":"wikcn..."}' \
  -- wiki spaces get_node --as user --format json

python scripts/lark_cli_json.py \
  --json-arg data='{"name":"空间名称","description":"空间描述"}' \
  -- api POST /open-apis/wiki/v2/spaces --as user

PowerShell-friendly pattern:

$env:LARK_JSON='{"token":"wikcn..."}'
python scripts/lark_cli_json.py \
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="NAME=JSON",
        help="append a JSON-valued CLI flag such as params={...} or data={...}",
    )
    parser.add_argument(
        "--json-env",
        action="append",
